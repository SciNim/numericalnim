...
converter toProc*[T](spline: InterpolatorType[T]): proc(x: float): T =
  result = proc(t: float): T = eval(spline, t)

converter toOptionalProc*[T](spline: InterpolatorType[T]): proc(x: float, optional: seq[T] = @[]): T =
...
proc toDerivProc*[T](spline: InterpolatorType[T]): proc(x: float): T =
  result = proc(t: float): T = derivEval(spline, t)

proc toDerivOptionalProc*[T](spline: InterpolatorType[T]): proc(x: float, optional: seq[T] = @[]): T =
